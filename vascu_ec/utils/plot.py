...
def plot_marker(parameters, im_marker, masks, single_cell_props, filename, output_path):
    ...
    for index, row in single_cell_props.iterrows():
        ...
        if sub_figs > 2:
            ax[sub_figs - 1].text(row["Y_nuc"], row["X_nuc"], str(np.round(row["mean_expression_nuc"], 1)), color="w",
                                  fontsize=6)
...
def plot_alignment(parameters, im_junction, masks, single_cell_props, filename, output_path):
    ...
    if sub_figs > 1:
        ...
        cax_0 = ax[0].imshow(np.ma.masked_where(cell_mask == 0, cell_eccentricity), cmap=plt.cm.bwr, vmin=0, vmax=1.0,
                             alpha=0.5)
